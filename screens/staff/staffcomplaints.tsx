...
export default function StaffComplaints({ onBack }: { onBack?: () => void }) {
  ...
  const load = React.useCallback(async (status: 'pending' | 'resolved', isRefresh = false) => {
    try {
      ...
      const url = `${BASE_URL}/api/staff/complaints?status=${encodeURIComponent(status)}`;
  ...
  const resolveComplaint = React.useCallback(async (id: string) => {
    try {
      setResolvingId(id);
      const token = await getAuthToken();
      const url = `${BASE_URL}/api/staff/complaints/${encodeURIComponent(id)}/resolve`;
      const resp = await fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      setError(e?.message || 'Failed to resolve complaint');
    } finally {
      setResolvingId(null);
    }
  }, [tab, load]);
              ...
              <View style={{ gap: 12 }}>
                {!!error && (
                  <View style={{ paddingVertical: 8 }}>
                    <Text style={{ color: '#b00020', fontWeight: '700' }}>Error: {error}</Text>
                  </View>
                )}
                {items.map((it) => (
                  <View key={it.id} style={styles.card}>
                    ...
                    {!!it.message && !expanded.has(it.id) && (
                      <View style={styles.cardBody}><Text style={styles.noteText} numberOfLines={2}>{it.message}</Text></View>
                    )}
                      ...
                      <View style={styles.detailsBox}>
                        {!!it.message && (
                          <View style={styles.detailLine}>
                            ...
                            <Text style={styles.detailValue}>{it.message}</Text>
                          </View>
                        )}
                        {!!it.lead?.phone && (
                          <View style={styles.detailLine}>
                        ...
                        {String(it.status).toLowerCase() !== 'resolved' && (
                          <View style={{ marginTop: 8, flexDirection: 'row', justifyContent: 'flex-end' }}>
                            <Pressable
                              onPress={() => resolveComplaint(it.id)}
                              disabled={resolvingId === it.id}
                              accessibilityRole="button"
                              style={[styles.resolveBtn, resolvingId === it.id && { opacity: 0.6 }]}
                            >
                              <Text style={styles.resolveBtnText}>{resolvingId === it.id ? 'Resolving…' : 'Resolve'}</Text>
                            </Pressable>
                          </View>
                        )}
                      </View>
                ...
                {items.length === 0 && !error && (
                  <View style={{ paddingVertical: 16, alignItems: 'center' }}>
                    <Text style={{ color: 'rgba(28,28,30,0.7)', fontWeight: '700' }}>No complaints found</Text>
                  </View>
                )}
              </View>
            )}
